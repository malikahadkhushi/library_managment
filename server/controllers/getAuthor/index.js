const { findAuthor } = require("../../services/Authors");

exports.getAuthorsByText = async (req, res) => {
  ...
  try {
    ...
    const { count, rows } = await findAuthor({ searchText, offset, limit });
    ...
    if (!rows.length) {
      res.status(401).send("Author is not Found");
    } else {
